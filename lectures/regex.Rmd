...
pacman::p_load(pdftools,     # extract content from PDF documents
               ...
               qdap)         # Tools for qualitative data
```
...
The following are simple examples showing how to use anchor tokens to find and replace sequences. 
...
The following examples show how to use the anchor syntax to match character classes.  For information on the `grep` function used in this example visit the main regex functions section.
...
The following examples show how to use the anchor syntax to match POSIX character classes. For information on the `grep` function used in this example visit the [main regex functions section](#regex_functions_base).
...
files <- list.files(root, pattern = '17-965')

trav_ban_pdf <- file.path(root,
                          str_subset(files, pattern = '.pdf'))
...
trav_ban_txt <- file.path(root,
                          str_subset(files, pattern = '.txt'))
...
file_root <- here::here('raw_data_files', 'supreme_court_opinions_2017')

# Assign the file URL to a text string
pdf <- file.path(file_root, '17-965_h315.pdf')
```
